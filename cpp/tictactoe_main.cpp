...
static void try_tictactoe_print(const tictactoe &ttt)
{
    errno = 0;
    if (ttt.print())
    {
        PRINTF_EXIT___FATAL_ERROR___FUNCTION_FAILURE(&tictactoe::print);
    }
}
...
static int try_tictactoe_who_won(const tictactoe &ttt)
{
    int winner_value;

    errno = 0;
    if (ttt.who_won(&winner_value))
    {
        PRINTF_EXIT___FATAL_ERROR___FUNCTION_FAILURE(&tictactoe::who_won);
    }
...
static void play_game(void)
{
    ...
    while (1)
    {
        ...
        while (1)
        {
            ...
            errno = 0;
            if (ttt.set_cell(row, col, cell_value))
            {
