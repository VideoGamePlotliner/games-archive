...
int tictactoe_init(struct tictactoe *ttt)
{
    if (!ttt)
    {
        PRINTF___OTHER_ERROR___VARIABLE_IS_NULL(ttt);
        errno = EINVAL;
        return -1;
    }
...
int tictactoe_print(const struct tictactoe *ttt)
{
    ...
    for (int row = 0; row < TICTACTOE_SIZE; row++)
    {
        ...
        for (int col = 0; col < TICTACTOE_SIZE; col++)
        {
            ...
            if (tictactoe_get_cell(ttt, row, col, &cell_value))
            {
                PRINTF___OTHER_ERROR___FUNCTION_FAILURE(tictactoe_get_cell);
                return -1;
            }
            printf(" %c ", cell_value);
        }
...
int tictactoe_set_cell(struct tictactoe *ttt, int row, int col, char cell_value)
{
    ...
    ttt->board[row][col] = cell_value;
    return 0;
}

/**
 * This function is only to be called by `tictactoe_who_won()`.
 *
...
int tictactoe_who_won(const struct tictactoe *ttt, int *winner_value)
{
    ...
    for (int row = 0; row < TICTACTOE_SIZE; row++)
    {
        ...
        for (int col = 0; col < TICTACTOE_SIZE; col++)
        {
            char cell_value = ttt->board[row][col];
